			    ...
			    $(document).ready(function(){					
					$("#jquery_jplayer_1").jPlayer({
					  ready: function () {
						$(this).jPlayer("setMedia", {
						  title:"<?php $_GET['title']; ?>",  
						  <?php echo (substr(strrchr($preview_file,'.'),1) == 'mp4')
													? 'm4v'
														: substr(strrchr($preview_file,'.'),1);?>: "<?php echo $preview_file;?>",
						  m4v: "<?php echo $preview_file; ?>"//,
					  ...
					  solution:"html,flash",
					 // supplied: "mp4, m4v, ogv, flv, webmv, webm",
					 supplied: "<?php echo (substr(strrchr($preview_file,'.'),1) == 'mp4')
													? substr(strrchr($preview_file,'.'),1)
