...
echo Form::hidden('id', @$fields['id'],array('id'=>'id'));
echo Form::hidden('current_order', @$fields['order'],array('id'=>'current_order'));
?>	
	<div class="form_wrapper">	
        ...
        <div class="form_row">
            ...
            <select name="category_id" id="category_id" class="required" data-url="<?php echo URL::site(ADMIN.$class_name.'/cid_order');?>">
                <option value="">&nbsp;</option>
                <?php foreach ($categories as $row) : ?>
                <option value="<?php echo ($row->status == 'publish') ? $row->id : '0';?>" <?php echo ($row->status != 'publish') ? 'disabled="disabled"' : '';?> <?php echo ($row->id == $fields['category_id']) ? 'selected="selected"' : ''; ?>>
					<?php echo str_repeat('&nbsp;', $row->sub_level * 5).HTML::chars($row->title, TRUE); ?> 
					<?php echo !empty($row->subject) ? '('. $row->subject.')' :''?>
					<?php echo ($row->status != 'publish') ? '('.$row->status.')' : ''; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <?php else : ?>
		...
		<?php endif; ?>
			<div class="ls10 clear"></div>			
				<div class="form_row">
				<label for="top_brand">Top Brand</label>
				<?php echo Form::checkbox('top_brand', 1, (!empty($fields['top_brand']) ? TRUE : FALSE), array('id'=>'top_brand')); ?>
			</div>
		</fieldset>
			...
			foreach($language_data as $language) :
				...
				<div id="lang_<?php echo $prefix[$language->id]; ?>">
					...
					<div class="form_row">
						...
						<textarea name="detail[<?php echo $language->id;?>][overview]" id="overview_<?php echo $detail_data[$language->id]->lang_id; ?>_<?php echo $detail_data[$language->id]->lang_id; ?>" class="ckeditor"><?php echo $detail_data[$language->id]->overview; ?></textarea>
					</div>
					<div class="form_row">
						<label>Features</label>
						<textarea name="detail[<?php echo $language->id;?>][features]" id="features_<?php echo $detail_data[$language->id]->lang_id; ?>_<?php echo $detail_data[$language->id]->lang_id; ?>" class="ckeditor"><?php echo $detail_data[$language->id]->features; ?></textarea>
					</div>
					<div class="form_row">
						<label>Specification</label>
						<textarea name="detail[<?php echo $language->id;?>][specification]" id="specification_<?php echo $detail_data[$language->id]->lang_id; ?>_<?php echo $detail_data[$language->id]->lang_id; ?>" class="ckeditor"><?php echo $detail_data[$language->id]->specification; ?></textarea>
					</div>
				</div>	
				<?php 
					else:
				...
				<div id="lang_<?php echo $prefix[$language->id]; ?>">
					...
					<div class="form_row">
						<label>Overview</label>
						<textarea name="detail[<?php echo $language->id;?>][overview]" id="overview_<?php echo $language->id; ?>" class="ckeditor"></textarea>
					</div>
					<div class="form_row">
						<label>Features</label>
						<textarea name="detail[<?php echo $language->id;?>][features]" id="features_<?php echo $language->id; ?>" class="ckeditor"></textarea>
					</div>
					<div class="form_row">
						<label>Specification</label>
						<textarea name="detail[<?php echo $language->id;?>][specification]" id="specification_<?php echo $language->id; ?>" class="ckeditor"></textarea>
					</div>
				</div>
			<?php
				//endif;
