...
<form class="form_details" action="<?php echo url::site(ADMIN.$class_name.'/edit/'.$product->id); ?>" method="get">
		...
		<fieldset class="wrapper_fs"><legend>Title Product</legend>
			<div class="form_row">
				...
				<div class="form_fields"><?php echo ($product->title) ? $product->title : '-'; ?></div>
			</div>
			<div class="form_row">
				<label><?php echo i18n::get('status');?></label>
				<div class="form_fields"><?php echo ucfirst($product->status);?></div>
			</div>	
		...
		<fieldset class="wrapper_fs"><legend>Detail Language</legend>
			...
			<?php 
				...
				foreach($language_data as $language):						
					...
					<div id="lang_<?php echo $prefix[$language->id]; ?>">
						...
						<div class="form_row">
							<div class="cd_left gray">Overview</div>
							<div class="cd_center">:</div>
							<div class="clear ls4"></div> 
							<div class="form_field"><?php echo $detail_data[$language->id]->overview; ?></div>
						</div>
						<div class="form_row">
							<div class="cd_left gray">Features</div>
							<div class="cd_center">:</div>
							<div class="clear ls4"></div> 
							<div class="form_field"><?php echo $detail_data[$language->id]->features; ?></div>
						</div>
						<div class="form_row">
							<div class="cd_left gray">Specification</div>
							<div class="cd_center">:</div>
							<div class="clear ls4"></div> 
							<div class="form_field"><?php echo $detail_data[$language->id]->specification; ?></div>
						</div>
					</div>	
				<?php
					endif;
			...
			<div class="form_row topBotDiv10">
				...
				<?php 					
				if (strpos($product->media,'/videos/') === 0) {
					?>
					<script type="text/javascript">
					//<![CDATA[
					$(document).ready(function(){
						$("#jquery_jplayer_1").jPlayer({
							ready: function () {
								$(this).jPlayer("setMedia", {
									title: "<?php echo $product->media;?>",
									<?php echo (substr(strrchr($product->media,'.'),1) == 'mp4')
												? 'm4v'
													: substr(strrchr($product->media,'.'),1);?>: "<?php echo URL::site('uploads/download_files'.$product->media);?>"
								});
							},
							swfPath: base_URL+"assets/js/library/jPlayer/",
							supplied: "<?php echo (substr(strrchr($product->media,'.'),1) == 'mp4')
												? 'm4v'
													: substr(strrchr($product->media,'.'),1);?>",
							errorAlerts: true,
							size: {
								width: "640px",
								height: "360px",
				...
				if(preg_match('/iframe/',$product->media) == '') {
					...
					$videoid = preg_replace($search, $replace, $product->media );
					if (!empty($videoid)) { 
						echo $videoid;
					}
				} else {
			...
			<div class="form_row">
				<label>Order</label>
				<div class="form_field"><?php echo $product->order;//$order; ?></div>
			</div>
			<?php endif; ?>			
			<div class="form_row">
				<label>Status</label>
