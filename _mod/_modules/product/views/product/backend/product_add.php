...
?>
		...
		<fieldset>		
			...
			<div class="form_row">
				...
				<select name="category_id" id="category_id" class="required" data-url="<?php echo URL::site(ADMIN.$class_name.'/cid_order');?>">
					...
					<option value="<?php echo ($row->status == 'publish') ? $row->id : 0;?>" <?php echo ($row->status != 'publish') ? 'disabled="disabled"' : '';?> <?php echo ($row->id == $fields['category_id']) ? 'selected="selected"' : ''; ?>><?php echo str_repeat('&nbsp;', $row->sub_level * 5).HTML::chars($row->title, TRUE); ?>
        ...
        <?php foreach ($uploads as $row_name => $row_params) : ?>
            <fieldset style="clear:both;" class="topDiv30">
		...
		<div class="form_row">
			<label>Order</label>
			<input type="text" style="width:18px;" name="order" id="order" value="<?php echo $fields['order'];?>" class="required"/>				
		</div>
		<?php endif; ?>
