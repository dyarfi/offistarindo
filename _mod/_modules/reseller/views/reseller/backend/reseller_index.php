		...
		<tbody id="sortable">
			...
			?>
				<tr id="row_<?php echo $row->id; ?>" class="<?php echo ($i % 2) ? 'even_row' : 'odd_row'; ?> <?php echo ($row->status != $statuses[0]) ? 'red_row' : ''; ?>">
					...
					<td class="td_number"><?php echo $i; ?></td>
					<td class="td_show" data-url="<?php echo URL::site(ADMIN.$class_name.'/order/action/' . $row->id.'/'.$row->order.'/?xhr=1');?>">						<strong><a href="<?php echo URL::site(ADMIN.$class_name.'/view/' . $row->id); ?>">
							<?php 
							echo !empty($detail_data[$row->id]) ? Text::limit_words(strip_tags($detail_data[$row->id]->subject),6).' ('.Text::limit_chars(strip_tags($row->title),16).')' : ' (no language detail)'; ?></a></strong>
						...
						<div class="show_order">
							<!--div style="position: absolute"><?php echo $row->order;?></div-->
							<?php 
