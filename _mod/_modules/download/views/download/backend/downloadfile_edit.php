...
<div class="form_wrapper">	
		...
		<fieldset>		
			...
			<div class="form_row">
				...
				<select name="product_id" id="product_id" class="required">
					...
					<?php
					if (count($products) == 1) {?>
					<option value="<?php echo $products->id; ?>" class="category_<?php echo $products->id; ?>" <?php echo ($fields['product_id'] == $products->id) ? 'selected="selected"' : ''; ?>>
					<?php echo Text::limit_words(HTML::chars($products->subject, TRUE),8);?>
					</option>
					<?php
					} else {
					...
					<?php echo Text::limit_words(HTML::chars($product->subject, TRUE),8);?>
					</option>
					<?php 
					endforeach;
					}
	...
	<?php if (isset($show_upload) && $show_upload) : ?>
        <?php foreach ($uploads as $row_name => $row_params) : ?>
            <fieldset style="clear:both;">
                ...
                <?php if (isset($filesFile[$row_name])) : ?>
                <div class="form_row">
                    <label><?php echo $row_params['label']; ?></label>
                    <div class="form_fields">
                        <?php if (is_file($upload_path.$filesFile[$row_name]->file_name) && in_array($filesFile[$row_name]->file_type, $readable_mime)) : ?>
                        <div id="file_<?php echo $filesFile[$row_name]->id; ?>">
                            <?php if (substr($filesFile[$row_name]->file_type, 0, strlen('image/')) == 'image/') : ?>
                            <?php
                                $file_data	= pathinfo(url::base().$upload_url.$filesFile[$row_name]->file_name);
                                $thumb_ext	= isset($row_params['image_manipulation']['thumbnails'][2]) ? '_resize_'.$row_params['image_manipulation']['thumbnails'][2] : '';
                            ?>
                            <img src="<?php echo url::base().$upload_url.$file_data['filename'].$thumb_ext.'.'.$file_data['extension']; ?>" alt="<?php echo url::base().Lib::config('downloadfile.upload_url').$filesFile[$row_name]->file_name; ?>" />
                            <?php elseif (substr($filesFile[$row_name]->file_type, 0, strlen('audio/')) == 'audio/') : ?>
                            <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="320" height="20" id="FLVPlayer">
                                ...
                                <param name="FlashVars" value="file=<?php echo URL::site(ADMIN.$module_name.'/download/'.$filesFile[$row_name]->file_name); ?>&amp;backColor=c2c2c2&amp;frontColor=666666&amp;showDownload=false&amp;repeatPlay=false&songVolume=100" />
                            ...
                            <object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="320" height="240" id="FLVPlayer">
                                ...
                                <param name="scale" value="noscale" />
                                <param name="salign" value="lt" />
                                <param name="FlashVars" value="skinName=<?php echo url::base(); ?>flash/Corona_Skin_2&amp;streamName=<?php echo URL::site(ADMIN.$module_name.'/download/'.$filesFile[$row_name]->file_name); ?>&amp;autoPlay=false&amp;autoRewind=false" />
                                <param name="swfversion" value="8,0,0,0" />
                                <!-- This param tag prompts users with Flash Player 6.0 r65 and higher to download the latest version of Flash Player. Delete it if you don�t want users to see the prompt. -->
                                <param name="expressinstall" value="Scripts/expressInstall.swf" />
                                <!-- Next object tag is for non-IE browsers. So hide it from IE using IECC. -->
                ...
                <div class="form_row">
					...
					<a href="<?php echo URL::site(ADMIN.$class_name.'/download/'.$filesFile[$row_name]->file_name); ?>">
						<img src="<?php echo IMG; ?>admin/disk.png" alt="<?php echo $filesFile[$row_name]->file_name; ?>" />
					</a>
					</div>
                </div>
                ...
                <div class="form_row">
                    ...
                    <?php if (isset($row_params['note']) && $row_params['note'] != '') : ?>
                        <div class="form_row">
                            <!--label>&nbsp;</label-->
                            <?php echo HTML::chars($row_params['note'], ENT_QUOTES); ?>
                        </div>
                    <?php endif; ?>
                </div>
	...
	<div class="form_row">
		...
		<a type="button" class="btn iframe-btn" href="<?php echo Lib::config('admin')->filemanager_path.'&amp;type=2&amp;&fldr=/uploads/';?>">Select</a>
		</div>
	</div>
	<?php } else if (!file_exists($upload_path.$file->file_name)) { ?>
	<div class="form_row">
		<label>File <span class="red">* File not found</span></label>			
		<div class="input-append">
		<input type="text" value="<?php echo $fields['file_name'];?>" readonly id="fileName" name="file_name" class="required">
...
$(document).ready(function() {
	<?php 
	$i = 1;
	foreach ($upload_pref as $pref => $val) { ?>
