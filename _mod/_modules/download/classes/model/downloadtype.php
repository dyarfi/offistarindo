...
class Model_DownloadType extends Model_Database {
	...
	public function delete ($id = '') {
		...
		$delete		= DB::delete('download_type_content')->where('type_id', '=', $id)->execute();
	...
	public function find_detail($where_cond = '', $order_by = '', $limit = '', $offset = '') {		
		...
		foreach ($detail as $data) {			
			...
			unset($_data_vars[$i]['_model_vars'],
				  ...
				  $_data_vars[$i]['_db']);			
			// Get Language Detail			
			$detail_data = Model::factory('DownloadTypeContent')->find(array('type_id'=>$data->id,'lang_id'=>Lang::_lang_id(I18n::lang())));			foreach ($detail_data as $detail) {
