...
class Model_Solution extends Model_Database {
	...
	public function load_by_name ($name = '') {
		...
		$objects	= $this->find(array('name' => $name,'status'=>'publish'), '', 1);
	...
	public function load_by_group ($id = '') {
		...
		$objects	= $this->find(array('category_id' => $id), '', 1);
	...
	public function delete ($id = '') {
		...
		$delete		= DB::delete('solution_content')->where('solution_id', '=', $id)->execute();
	...
	public function find_detail($where_cond = '', $order_by = '', $limit = '', $offset = '') {		
		...
		foreach ($detail as $data) {			
			...
			unset($_data_vars[$i]['_model_vars'],
				  ...
				  $_data_vars[$i]['_db']);			
			// Get Language Detail			
			$detail_data = Model::factory('SolutionContent')->find(array('solution_id'=>$data->id,'lang_id'=>Lang::_lang_id(I18n::lang())));
