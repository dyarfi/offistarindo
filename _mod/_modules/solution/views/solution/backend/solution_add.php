		...
		<fieldset>		
			...
			<div class="form_row">
				...
				<select name="category_id" id="category_id" class="required">
					...
					<option value="<?php echo ($row->status == 'publish') ? $row->id : 0;?>" <?php echo ($row->status != 'publish') ? 'disabled="disabled"' : '';?> <?php echo ($row->id == $fields['category_id']) ? 'selected="selected"' : ''; ?>><?php echo str_repeat('&nbsp;', $row->sub_level * 5).HTML::chars(Text::limit_chars($row->title,20), TRUE).' ('. Text::limit_chars($row->subject,20).')'; ?> <?php echo ($row->status != 'publish') ? '('.$row->status.')' : ''; ?></option>
					<?php endforeach; ?>
				</select>
			</div>
			<?php else : ?>
			...
			<div class="form_row">
				<label>Order</label>
				<input type="text" style="width:20px;" name="order" id="order" value="<?php echo $fields['order'];?>"/>
			</div>
			<?php endif; ?>
		</fieldset>
