...
class Model_TranslationTable extends Model_Database {

	protected $table = 'language_translation_tables'; 
	protected $tbl_name;
	...
	public function install () {
		$sql	= 'CREATE TABLE IF NOT EXISTS `'.$this->table.'` ('
				...
				. '`table_translate` VARCHAR(128) NULL, '
	...
	public function load_by_table_translate ($table) {
		if (!$table) {
			return FALSE;
		}
		$where_cond	= array('table_translate'	=> $table);
	...
	public function find ($where_cond = '', $order_by = '', $limit = '', $offset = '') {
		...
		foreach ($rows as $row) {
			$object			= new Model_TranslationTable;
