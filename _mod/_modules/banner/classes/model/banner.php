...
class Model_Banner extends Model_Database {
	...
	public function delete ($id = '') {
		...
		$delete		= DB::delete('banner_content')->where('banner_id', '=', $id)->execute();
	...
	public function find_detail($where_cond = '', $order_by = '', $limit = '', $offset = '') {		
		...
		foreach ($detail as $data) {			
			...
			unset($_data_vars[$i]['_model_vars'],
				  ...
				  $_data_vars[$i]['_db']);			
			// Get Language Detail			
			$detail_data = Model::factory('BannerContent')->find(array('banner_id'=>$data->id,'lang_id'=>Lang::_lang_id(I18n::lang())));
	...
	public function set_order($category_id = '', $group = '', $action='') {
		...
		if (!empty($category_id)) {
			$type = 'WHERE `category_id` = '. $parent_id .'';
		} 
