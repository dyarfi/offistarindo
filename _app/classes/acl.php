...
class Acl extends Controller {
	...
	public function __construct () {
		...
		if ($this->user === FALSE  
			...
			/*
			!empty($this->previous_url) 
				? $this->request->redirect($this->previous_url) 
					: $this->request->redirect(ADMIN . 'authentication');
			*/
		}	
	}
	...
	public function check_module_permission ($controller='',$action='') {
		...
		if ($module_list && $module_function_list && strstr($this->request->detect_uri(), ADMIN) !='') {
			...
			if ($disable_accessible === FALSE 
					...
					&& $action != 'check_title'
					// For Bypassing admin-panel check_order method in all classes
					&& $action != 'check_order'
					...
					&& $action != 'top_brand'
					// For Bypassing admin-panel top brand method in all classes					
					&& $action != 'download'
